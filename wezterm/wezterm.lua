...
wezterm.on("open-uri", function(_, pane, uri)
  ...
  if stdout:find("directory") then
    pane:send_text(carriage_return(wezterm.shell_join_args({ "cd", url.file_path })))
    pane:send_text(
      carriage_return(wezterm.shell_join_args({ "ls", "-A", "-p", "--group-directories-first" }))
    )
    return false
  end
  if stdout:find("text") then
    local text = not url.fragment and wezterm.shell_join_args({ editor, url.file_path })
      or carriage_return(wezterm.shell_join_args({ editor, "+" .. url.fragment, url.file_path }))

...
_luarocks() {
    ...
    case "$cmd" in
        ...
        'help') COMPREPLY=($(compgen -W "help completion build config doc download init install lint list make new_version new-version pack path purge remove search show test unpack upload which write_rockspec write-rockspec" -- "$cur")) ;;
        'build')
            case "$prev" in
                --branch)
                    COMPREPLY=($(compgen -f -- "$cur"))

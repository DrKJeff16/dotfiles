...
function os.capture(cmd, _)
  local f = assert(io.popen(cmd, 'r'))
  local s = assert(f:read('*a'))
  f:close()
  return s:sub(0, -2) -- Use string.sub to trim the trailing newline.
end
...
function delete_current_track()
  ...
  os.execute(("mv '%s' '%s'"):format(last_deleted_track, deleted_tmp))
  print(("'%s' deleted."):format(last_deleted_track))
end
